...
export class Room{
  ...
  private createPeer(initiator:boolean){ const peer=new Peer({initiator,trickle:true,config:{iceServers:this.options.iceServers},channelName:'game'}); let connected=false;
    const fallback=setTimeout(()=>{ if(!connected){ this.useRelay(); } },7000);
    peer.on('signal',(sig:any)=>{ if(this.ws&&this.ws.readyState===WebSocket.OPEN){ this.ws.send(JSON.stringify({type:'signal',payload:sig})); } });
    ...
    peer.on('data',(d:any)=>this.onPeerData?.(d)); peer.on('close',()=>this.onPeersChange?.(this.peers.length)); peer.on('error',(e)=>console.error('[peer] error',e));
